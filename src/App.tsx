...
export default function App() {
  ...
  const featuredFilms = works.filter(work => 
    ['5', '7', '10'].includes(work.id) // Status Change, Canopy, 3 Ghost Stories
  )
  
  const featuredProducts = products.filter(product => 
    ['9', '1', '5'].includes(product.id) // Spike, Dune-Ripples, Bladerunner
  )
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      {currentView === 'admin' && (
        <AdminDashboard
          ...
          onBackToSite={() => handleNavigation('home')}
        />
      )}
        ...
        <>
          ...
          {currentView === 'home' && (
            <>
              ...
              <WorksSection 
                works={featuredFilms} 
                onWorkClick={handleFilmClick}
                title="Featured Films"
                showSeeAll={true}
                onSeeAllClick={() => handleNavigation('films')}
              />
              <ProductsSection 
                products={featuredProducts}
                title="Featured Sounds"
                showSeeAll={true}
                onSeeAllClick={() => handleNavigation('sounds')}
              />
            </>
          )}
          ...
          {currentView === 'film-detail' && selectedWork && (
            <WorkDetail work={selectedWork} onBack={() => handleNavigation('films')} />
          )}
          
          {currentView === 'about' && (
            <AboutSection onBack={() => handleNavigation('home')} />
          )}
