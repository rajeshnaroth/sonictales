      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product) => (
            <div
              key={product.id}
              className="group bg-gray-900/50 rounded-xl overflow-hidden hover:bg-gray-900/70 transition-colors"
            >
              {/* Square thumbnail */}
              <div className="relative aspect-square bg-gray-900 overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <span className={`px-3 py-1 rounded-full text-xs tracking-wide backdrop-blur-sm ${
                    product.other_metadata.free 
                ...
                {product.file_size && (
                  <div className="absolute top-4 right-4">
                    <span className="px-2 py-1 bg-black/70 backdrop-blur-sm rounded text-xs text-white/80">
                      {product.file_size}
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`text-sm ${
                      product.other_metadata.free ? 'text-green-400' : 'text-blue-400'
                    }`}>
                      {product.other_metadata.free ? 'Free Download' : 'Purchase'}
                    </span>
                  </div>
        ...
        <div className="mt-16 text-center">
          <div className="bg-gray-900/50 rounded-xl p-8 max-w-2xl mx-auto">
            ...
            <p className="text-white/70 mb-6">
              Discover more sound design resources and stay updated with our latest releases.
            </p>
            <a
